/*Write a program to perform create at start, create at end, create at given location, 
delete at start, delete at end, delete at given location and traverse for a singly linked list 
with two external reference pointers start and last.*/
...
void deleteAtLoc()  // function to delete the node at certain location as wish by the user
{
	...
	else
	{
		...
		if (p != NULL) // if the location is valid 
		{
			q->next = p->next;
			if(p->next == NULL)
			{
